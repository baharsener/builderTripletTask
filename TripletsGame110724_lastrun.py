...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Display\\Desktop\\builderTripletTask\\TripletsGame110724_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Unknown component ignored: StartRecord_Encoding1
    ...
    fix = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0), size=(0.07, 0.06),
    ...
    mask_left = visual.ImageStim(
        ...
        ori=0.0, pos=(-0.5, 0.0), size=(0.25, 0.25),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    mask_mid = visual.ImageStim(
        win=win,
        name='mask_mid', 
        image='images/Mask.png', mask=None, anchor='center',
    ...
    # Unknown component ignored: HostDrawing_Ret1
    
    
    # Unknown component ignored: StartRecord_Ret1
    ...
    # Unknown component ignored: MarkEventsQ2Text
    ...
    # Unknown component ignored: MarkEvents_Ret1_RetImg
    ...
    # Unknown component ignored: MarkEvents_Ret1_Decision
    ...
    # Unknown component ignored: DriftCheck_2
    ...
    # Unknown component ignored: MarkEvents_Ret2_RetImg
    ...
    # Unknown component ignored: MarkEvents_Ret2_Decision2
    ...
    allPics = [f"OBJ_{i}" for i in range(1, 211)];
    sameDiff = ["same"] * 20 + ["diff"] * 5 #25 trials with 20 'same', 5 'diff'
    shuffle(allPics) #shuffle image list
    shuffle(sameDiff) #shuffle trial order list
    print(sameDiff)
    #For the trial order:
    #Define the strings
    fixed_strings = ['LMR', 'RML']  #These will occur 9 times each
    other_strings = ['MLR', 'MRL', 'LRM', 'RLM']  #These will be randomly selected to have total of 9 also
    ...
    sameTrials = (fixed_strings * 9) + selected_strings
    ...
    diffTrials = (fixed_strings * 9) + selected_strings
    random.shuffle(diffTrials)
    print("'different' trials:")
    print(diffTrials) 
    
    numItems = len(allPics)
    ...
    #Split allPics into triplets
    trialPics = [allPics[i:i+3] for i in range(0, numItems, 3)] #makes 70 triplets from shuggled images
    if len(trialPics[-1]) != 3:
        trialPics.pop()  #Remove the last item if it's not a full triplet
    print("Total number of triplets created:", len(trialPics))
        
    #Set for the two encoding blocks
    encodingPics1 = trialPics[:25]  #First 25 triplets for block 1
    encodingPics2 = trialPics[25:50]  #Next 25 triplets for block 2
    
    #Print the selected encoding triplets
    print("Encoding Block 1 Triplets:")
    print(encodingPics1)
    print("Encoding Block 2 Triplets:")
    print(encodingPics2)
    
    #Initialize positiom lists of encoding images for retrieval img balance
    left_encoded_pics = []
    middle_encoded_pics = []
    right_encoded_pics = []
    
    #Initialize for final retrieval lists:
    samePics = []
    samePics2 = []
    ...
    for thisEncodingTrial in EncodingTrials:
        ...
        # keep track of which components have finished
        startRecordingEncoding1Components = [HostDrawing_Encoding1, StartRecord_Encoding1]
        ...
        #position map
        position_map = {
            'LMR': (leftPos, midPos, rightPos),
            'LRM': (leftPos, rightPos, midPos),
            'MLR': (midPos, leftPos, rightPos),
            'RML': (rightPos, midPos, leftPos),
            'RLM': (rightPos, leftPos, midPos),
            'MRL': (midPos, rightPos, leftPos),
        }
        ...
        #get positions from the position map
        firstPos, secondPos, thirdPos = position_map[trialType]
        
        #set images in triplets for the first view
        ...
        #save first view positions and triplets
        firstPos1 = firstPos
        ...
        # keep track of which components have finished
        maskComponents = [mask_left, mask_mid, mask_right, MarkEvents_mask]
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # if mask_left is starting this frame...
            if mask_left.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                mask_left.frameNStart = frameN  # exact frame index
                mask_left.tStart = t  # local t and not account for scr refresh
                mask_left.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(mask_left, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'mask_left.started')
                # update status
                mask_left.status = STARTED
                mask_left.setAutoDraw(True)
            
            # if mask_left is active this frame...
            if mask_left.status == STARTED:
                # update params
                pass
            
            # if mask_left is stopping this frame...
            if mask_left.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mask_left.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    mask_left.tStop = t  # not accounting for scr refresh
                    mask_left.tStopRefresh = tThisFlipGlobal  # on global time
                    mask_left.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'mask_left.stopped')
                    # update status
                    mask_left.status = FINISHED
                    mask_left.setAutoDraw(False)
            
            # *mask_mid* updates
            
            # if mask_mid is starting this frame...
            if mask_mid.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                mask_mid.frameNStart = frameN  # exact frame index
                mask_mid.tStart = t  # local t and not account for scr refresh
                mask_mid.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(mask_mid, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'mask_mid.started')
                # update status
                mask_mid.status = STARTED
                mask_mid.setAutoDraw(True)
            
            # if mask_mid is active this frame...
            if mask_mid.status == STARTED:
                # update params
                pass
            
            # if mask_mid is stopping this frame...
            if mask_mid.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mask_mid.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    mask_mid.tStop = t  # not accounting for scr refresh
                    mask_mid.tStopRefresh = tThisFlipGlobal  # on global time
                    mask_mid.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'mask_mid.stopped')
                    # update status
                    mask_mid.status = FINISHED
                    mask_mid.setAutoDraw(False)
            
            # *mask_right* updates
            
            # if mask_right is starting this frame...
            if mask_right.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                mask_right.frameNStart = frameN  # exact frame index
                mask_right.tStart = t  # local t and not account for scr refresh
                mask_right.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(mask_right, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'mask_right.started')
                # update status
                mask_right.status = STARTED
                mask_right.setAutoDraw(True)
            
            # if mask_right is active this frame...
            if mask_right.status == STARTED:
                # update params
                pass
            
            # if mask_right is stopping this frame...
            if mask_right.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mask_right.tStartRefresh + 2-frameTolerance:
                    # keep track of stop time/frame for later
                    mask_right.tStop = t  # not accounting for scr refresh
                    mask_right.tStopRefresh = tThisFlipGlobal  # on global time
                    mask_right.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'mask_right.stopped')
                    # update status
                    mask_right.status = FINISHED
                    mask_right.setAutoDraw(False)
            if thisExp.status == FINISHED or endExpNow:
                endExperiment(thisExp, win=win)
                return
        ...
        #trial type and position order based on same/diff
        if method_Trial == "same":
            trialType = sameTrials[eTrials]
            position_order = position_map[trialType]
            methodAns = ['leftCL'] 
            ...
            #select a new trial that isn't the same as the previous one
            ...
            #shuffle list of indices for the triplet
            list_indices = [0, 1, 2]
            random.shuffle(list_indices)
        
            #assign triplets using the shuffled indices
            triplet1 = 'images/' + encodingPics1[eTrials][list_indices[0]] + '.png'
            triplet2 = 'images/' + encodingPics1[eTrials][list_indices[1]] + '.png'
            triplet3 = 'images/' + encodingPics1[eTrials][list_indices[2]] + '.png'
            
            #encoding type
            EncodingTrials.addData("encodingType", "diff")
            encodingType = 'diff'
        
        # Add data for the second view
        thisExp.addData("triplet1_second_view", triplet1)
        thisExp.addData("triplet2_second_view", triplet2)
        thisExp.addData("triplet3_second_view", triplet3)
    ...
    # Set trial numbers and initialize lists
    retTrials = 0
    numRetrieval1 = 20
    samePics = []  #list to hold all encoding images
    sampled_images = []  #list to hold all encoding images with their positions
    retrieval_list = []  #list to hold selected retrieval images
    
    #Set retrieval image selection logic
    sampled_trials = set()  #Track encoding trials we already sampled retrieval images from
    
    #Balance the number of images based on encoding position
    img_per_position = {"left": 7, "middle": 7, "right": 7}  #initially set each position to sample 7
    position_to_sample_less = random.choice(["left", "middle", "right"])  #randomly choose one to sample 6 from
    img_per_position[position_to_sample_less] = 6  #set chosen position to have only 6
    
    #Track counts for each position
    position_counts = {"left": 0, "middle": 0, "right": 0}  
    
    #Loop through encoding trials
    for eTrials in range(numEncodingBlock1):
        method_Trial = sameDiff[eTrials]  #Determine if the trial is "same" or "different"
    
        #Process only 'same' trials that haven't been sampled yet
        if method_Trial == "same" and eTrials not in sampled_trials:
            trialType = sameTrials[eTrials]  # get trial type
            position_order = position_map[trialType]  # get position order
    
            #Get images based on the position order
            images = {
                "left": encodingPics1[eTrials][position_order.index(leftPos)],
                "middle": encodingPics1[eTrials][position_order.index(midPos)],
                "right": encodingPics1[eTrials][position_order.index(rightPos)]
            }
    
            #Filter images based on current counts
            available_images = [
                (img, pos) for pos, img in images.items() 
                if position_counts[pos] < img_per_position[pos]
            ]
    
            #Choose an image if any are available
            if available_images:
                chosen_image, chosen_position = random.choice(available_images)
    
                #Append to retrieval data and update counts
                retrieval_list.append({"image": chosen_image, "encoding_position": chosen_position})
     #           samePics.append(chosen_image)
                sampled_trials.add(eTrials)  #Mark the trial as sampled
                position_counts[chosen_position] += 1  #Update position count
    #shuffle final list:
    random.shuffle(retrieval_list)
    print("shuffled retrieval list:")
    
    for item in retrieval_list: # `item` is a dictionary with "image" and "encoding_position" as keys
        image_name = item["image"]
        encoding_position = item["encoding_position"] 
        
        #put the shuffled images in samePics2 while you keep encode pos info
        samePics.append(image_name)
    # keep track of which components have finished
    ...
    for thisRetrievalTrial in RetrievalTrials:
        ...
        # keep track of which components have finished
        startRecordingRetrieval1Components = [HostDrawing_Ret1, StartRecord_Ret1]
        ...
        # keep track of which components have finished
        Q2TextComponents = [question_text, MarkEventsQ2Text]
        ...
        retrievalImage = None
        # Ensure `samePics` has remaining images 
        if len(samePics) == 0:
            sys.exit("Error: samePics is empty") 
        # Select and store the current retrieval image from samePics 
        retrievalImage = 'images/' + samePics[retTrials] + '.png'
        encode_position = str(retrieval_list[retTrials]["encoding_position"])
        
        # Add data
        trial_category = "retrieval"
        RetrievalTrials.addData("retIMG", retrievalImage)
        thisExp.addData("retIMG", retrievalImage)
        thisExp.addData("encode_position", encode_position)
        ...
        sound_pop7.seek(0)
        # keep track of which components have finished
        RetrievalImageComponents = [retIMG, sound_pop7, MarkEvents_Ret1_RetImg]
        ...
        win.mouseVisible = True
        el_tracker.sendMessage('decision started')
        
        #set timer for the mouse so that we can get the exact time of the decision
        Order_Position_Response.clickReset()
        mouse_clicked = False
        # setup some python lists for storing info about the Order_Position_Response
        Order_Position_Response.x = []
        Order_Position_Response.y = []
        Order_Position_Response.leftButton = []
        Order_Position_Response.midButton = []
        Order_Position_Response.rightButton = []
        Order_Position_Response.time = []
        Order_Position_Response.clicked_name = []
        Order_Position_Response.clicked_Order_Position_ResponseClock = []
        Order_Position_Response.clicked_getTime = []
        gotValidClick = False  # until a click is received
        Order_Position_Response.mouseClock.reset()
        # keep track of which components have finished
        DecisionComponents = [decisionQ_2, first_left, second_middle, third_right, First, Second, Third, Order_Position_Response, MarkEvents_Ret1_Decision]
        ...
        while continueRoutine:
            ...
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # Run 'Each Frame' code from dec_code
            #check every frame whether mouse is clicked
            if 1 in Order_Position_Response.getPressed() and not mouse_clicked:
                decMade = mouse.Order_Position_ResponseClock.getTime()
                mouse_clicked = True
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        for obj in clickableList:
                            ...
                            if obj.contains(Order_Position_Response):
                                ...
                                Order_Position_Response.clicked_Order_Position_ResponseClock.append(obj.Order_Position_ResponseClock)
                                Order_Position_Response.clicked_getTime.append(obj.getTime)
                        if gotValidClick:
                            x, y = Order_Position_Response.getPos()
                            Order_Position_Response.x.append(x)
                            Order_Position_Response.y.append(y)
                            buttons = Order_Position_Response.getPressed()
                            Order_Position_Response.leftButton.append(buttons[0])
                            Order_Position_Response.midButton.append(buttons[1])
                            Order_Position_Response.rightButton.append(buttons[2])
                            Order_Position_Response.time.append(Order_Position_Response.mouseClock.getTime())
                        if gotValidClick:
                            continueRoutine = False  # end routine on response
            if thisExp.status == FINISHED or endExpNow:
                endExperiment(thisExp, win=win)
        ...
        el_tracker.sendMessage('!V TRIAL_VAR decisionMade %d' % decisionMade)
        el_tracker.sendMessage('!V TRIAL_VAR encode_position %s' % encode_position)
        
        win.mouseVisible = False
        
        
        # store data for RetrievalTrials (TrialHandler)
        RetrievalTrials.addData('Order_Position_Response.x', Order_Position_Response.x)
        RetrievalTrials.addData('Order_Position_Response.y', Order_Position_Response.y)
        RetrievalTrials.addData('Order_Position_Response.leftButton', Order_Position_Response.leftButton)
        RetrievalTrials.addData('Order_Position_Response.midButton', Order_Position_Response.midButton)
        RetrievalTrials.addData('Order_Position_Response.rightButton', Order_Position_Response.rightButton)
        RetrievalTrials.addData('Order_Position_Response.time', Order_Position_Response.time)
        RetrievalTrials.addData('Order_Position_Response.clicked_name', Order_Position_Response.clicked_name)
        RetrievalTrials.addData('Order_Position_Response.clicked_Order_Position_ResponseClock', Order_Position_Response.clicked_Order_Position_ResponseClock)
        RetrievalTrials.addData('Order_Position_Response.clicked_getTime', Order_Position_Response.clicked_getTime)
    ...
    sameDiff2 = ["same"] * 20 + ["diff"] * 5
    random.shuffle(sameDiff2)
    print("Randomized same-different list 2:")
    print(sameDiff2)
    ...
    # Combine all strings for sameTrials
    sameTrials2 = fixed_strings * 9  # Repeating fixed_strings 9 times
    sameTrials2 = (fixed_strings * 9) + selected_strings
    random.shuffle(sameTrials2)
    
    # Print the randomized list
    print("Randomized string list (sameTrials2):")
    print(sameTrials2)
    
    # For the different trials:
    diffTrials2 = fixed_strings * 9  # Repeating fixed_strings 9 times
    diffTrials2 = (fixed_strings * 9) + selected_strings
    random.shuffle(diffTrials2)
    
    # Shuffle the list
    random.shuffle(diffTrials2)
    
    # Print the randomized list for diffTrials
    print("\nRandomized string list (diffTrials2):")
    print(diffTrials2)
    ...
    # Initialize positional storage for retrieval balance
    left_encoded_pics2 = []
    middle_encoded_pics2 = []
    right_encoded_pics2 = []
    # keep track of which components have finished
    init2Components = []
    ...
    for thisEncodingBlock2 in EncodingBlock2:
        ...
        # Save initial positions and triplets
        firstPos2 = firstPos
        ...
        if method_Trial == "same":
            ...
            # Shuffle the list of indices for the triplet
            list_indices = [0, 1, 2]
            random.shuffle(list_indices)
        
            # Assign triplets using the shuffled indices from encodingPics1
            triplet1 = 'images/' + encodingPics2[eTrials2][list_indices[0]] + '.png'
            triplet2 = 'images/' + encodingPics2[eTrials2][list_indices[1]] + '.png'
            triplet3 = 'images/' + encodingPics2[eTrials2][list_indices[2]] + '.png'
            
            # Log the encoding type for Block 2
    ...
    retTrials2 = 0
    numRetrieval2 = 20
    samePics2 = []  #list to hold all encoding images
    sampled_images2 = []  #list to hold all encoding images with their positions
    retrieval_list2 = []  #list to hold selected retrieval images
    
    #Set retrieval image selection logic
    sampled_trials2 = set()  #Track encoding trials we already sampled retrieval images from
    ...
    #Loop through encoding trials
    for eTrials2 in range(numEncodingBlock2):
        method_Trial = sameDiff2[eTrials2]  #Determine if the trial is "same" or "different"
    
        #Process only 'same' trials that haven't been sampled yet
        if method_Trial == "same" and eTrials2 not in sampled_trials2:
            trialType = sameTrials2[eTrials2]  # get trial type
            position_order = position_map[trialType]  # get position order
    
            #Get images based on the position order
            images = {
                "left": encodingPics2[eTrials2][position_order.index(leftPos)],
                "middle": encodingPics2[eTrials2][position_order.index(midPos)],
                "right": encodingPics2[eTrials2][position_order.index(rightPos)]
            }
            ...
            if available_images:
                ...
                #Append to retrieval list and update counts
                retrieval_list2.append({"image": chosen_image, "encoding_position": chosen_position})
    #            samePics2.append(chosen_image)
                sampled_trials2.add(eTrials2)  #Mark the trial as sampled
                position_counts[chosen_position] += 1  #Update position count
    #shuffle final list:
    random.shuffle(retrieval_list2)
    print("shuffled retrieval list2:")
    
    for item in retrieval_list2: # `item` is a dictionary with "image" and "encoding_position" as keys
        image_name = item["image"]
        encoding_position = item["encoding_position"] 
        
        #put the shuffled images in samePics2 while you keep encode pos info
        samePics2.append(image_name)
    # keep track of which components have finished
    ...
    # keep track of which components have finished
    eyeCheck2Components = [DriftCheck_2]
    ...
    for thisRetrievalBlock2 in RetrievalBlock2:
        ...
        if len(samePics2) == 0: 
            sys.exit("Error: samePics is empty") 
        # Select and store the current retrieval image from samePics 
        retrievalImage = 'images/' + samePics2[retTrials2] + '.png'
        encode_position = str(retrieval_list2[retTrials2]["encoding_position"])
        ...
        sound_pop7_2.seek(0)
        # keep track of which components have finished
        RetrievalImage2Components = [retIMG_2, sound_pop7_2, MarkEvents_Ret2_RetImg]
        ...
        win.mouseVisible = True
        el_tracker.sendMessage('decision 2 started')
        # setup some python lists for storing info about the Order_Position_Response_2
        Order_Position_Response_2.x = []
        Order_Position_Response_2.y = []
        Order_Position_Response_2.leftButton = []
        Order_Position_Response_2.midButton = []
        Order_Position_Response_2.rightButton = []
        Order_Position_Response_2.time = []
        Order_Position_Response_2.clicked_name = []
        gotValidClick = False  # until a click is received
        Order_Position_Response_2.mouseClock.reset()
        # keep track of which components have finished
        Decision2Components = [decisionQ_3, first_left_2, second_middle_2, third_right_2, First_2, Second_2, Third_2, Order_Position_Response_2, MarkEvents_Ret2_Decision2]
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        if gotValidClick:
                            ...
                            Order_Position_Response_2.x.append(x)
                            Order_Position_Response_2.y.append(y)
                            buttons = Order_Position_Response_2.getPressed()
                            Order_Position_Response_2.leftButton.append(buttons[0])
                            Order_Position_Response_2.midButton.append(buttons[1])
                            Order_Position_Response_2.rightButton.append(buttons[2])
                            Order_Position_Response_2.time.append(Order_Position_Response_2.mouseClock.getTime())
                        if gotValidClick:
                            continueRoutine = False  # end routine on response
            if thisExp.status == FINISHED or endExpNow:
                endExperiment(thisExp, win=win)
        ...
        #sanity check
        print("ret list 2:")
        print(retrieval_list2)
        print("samePics2:")
        print(samePics2)
        print("sampled trials 2:")
        print(sampled_trials2)
        ...
        RetrievalBlock2.addData('Order_Position_Response_2.x', Order_Position_Response_2.x)
        RetrievalBlock2.addData('Order_Position_Response_2.y', Order_Position_Response_2.y)
        RetrievalBlock2.addData('Order_Position_Response_2.leftButton', Order_Position_Response_2.leftButton)
        RetrievalBlock2.addData('Order_Position_Response_2.midButton', Order_Position_Response_2.midButton)
        RetrievalBlock2.addData('Order_Position_Response_2.rightButton', Order_Position_Response_2.rightButton)
        RetrievalBlock2.addData('Order_Position_Response_2.time', Order_Position_Response_2.time)
        RetrievalBlock2.addData('Order_Position_Response_2.clicked_name', Order_Position_Response_2.clicked_name)
